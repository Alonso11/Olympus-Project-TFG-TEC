...
Esta sección resume el Concepto de Operaciones (ConOps) del Rover Olympus y establece cómo se emplea el sistema en el ciclo de vida operativo, con foco en la ejecución del UC‑01 (Exploración de superficie) como caso núcleo de validación en entornos análogos.
...
El ConOps se utiliza como fuente primaria de derivación y justificación de requisitos (RF/RNF/CON) y como base para la planificación de Verificación y Validación (V\&V) del sistema, manteniendo trazabilidad hacia UC‑01 y los demás Casos de Uso top‑level.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=0.85, every node/.style={scale=0.7}]
        ...
        \node [phase] (landing)    at (0,0)     {Fase 1: Inicialización y Estabilización};
    ...
    \label{fig:mission_phases}
\end{figure}

El ciclo de vida operativo del Rover Olympus se estructura en fases (Fig.~\ref{fig:mission_phases}) que permiten organizar modos, responsabilidades y criterios de preparación para la validación del UC-01 en sitio análogo. Las fases se expresan como transiciones lógicas (no como \emph{fases de vuelo} reales), ya que Olympus es una plataforma terrestre de validación que emula el ciclo de una misión lunar de referencia conforme al Objetivo Específico 1 del TFG.

\begin{itemize}
    \item \textbf{Fase 1: Inicialización y Estabilización} (análoga al ``Aterrizaje y Estabilización'' de la misión lunar de referencia): Establecer condición ``en superficie'' del rover en el sitio análogo y estabilidad del sistema (energía, cómputo, comunicación).
    \item \textbf{Fase 2: Despliegue y Salida:} Habilitar subsistemas críticos (TRAC, PROP, COMMS, GNC, C\&DH) y preparar al rover para iniciar actividades en el entorno análogo.
    \item \textbf{Fase 3: Comisionamiento:} Calibraciones, chequeos funcionales, verificación de interfaces internas/externas y pruebas previas a operación nominal (pre‑V\&V).
    \item \textbf{Fase 4: Operación Nominal:} Ejecutar campañas de operación (incluyendo UC‑01 como caso principal), recolectar telemetría y evidencias V\&V, y operar bajo latencia simulada cuando aplique.
    \item \textbf{Fase 5: Supervivencia / Modo Seguro:} Preservar integridad del sistema ante condiciones adversas (p. ej., degradación de enlace, restricciones de energía, condiciones ambientales del sitio) y permitir recuperación a operación nominal cuando sea viable.
    \item \textbf{Fase 6: Fin de Misión (EOM / Decommissioning):} Cierre controlado de la campaña y transición a un estado final (ver UC-05 \emph{Safe Deactivation / Passivation} en \S\ref{sec:use_cases}).
\end{itemize}
...
Permite que el rover recorra una región de la superficie de forma autónoma, analizando el terreno, navegando de manera segura y recopilando telemetría y datos de navegación. Este caso de uso implica una alta autonomía y es el foco principal del presente trabajo.
...
El rover recibe desde el Operador una orden para desplazarse hasta un punto específico de interés. El sistema calcula y ejecuta la trayectoria hacia ese objetivo, utilizando sus capacidades de navegación y evitación de obstáculos.
  ...
  \item[\textbf{UC-03: Recolección de muestras}] \emph{[Conceptual --- fuera del alcance del prototipo TFG; sin actuador de recolección en la configuración actual.]}
...
Permite que el rover regrese a un punto seguro predefinido (por ejemplo, el punto base de despliegue inicial o una región protegida previamente declarada) cuando se detectan condiciones de riesgo (batería baja, condiciones ambientales adversas, pérdida de comunicación prolongada, etc.) o cuando el Operador lo ordena explícitamente.

  \item[\textbf{UC-05: Safe Deactivation / Passivation}]
Al final de la vida útil de la misión o ante un fallo crítico no recuperable, el rover ejecuta una secuencia controlada de \emph{passivation}: descarga segura de energía almacenada, desactivación de actuadores, persistencia del último estado en logs y entrada a un modo de mínima emisión. Esta práctica es coherente con las provisiones de \emph{Planetary Protection} para misiones robóticas \cite{nasa_planetary_protection} y reemplaza el concepto de ``autodestrucción'' (no aplicable a una plataforma terrestre de validación y desaconsejado para misiones espaciales por el riesgo de contaminación del sitio).
\end{description}
...
Aunque todos estos casos de uso son relevantes para la operación global del Rover Olympus, en este documento nos centraremos en el caso de uso \textbf{Exploración de superficie} (UC-01), el cual se detalla a continuación y sirve como base para el modelado de los diagramas de comportamiento; los demás (UC-02 a UC-05) se mantienen a nivel conceptual conforme a la decisión de \emph{tailoring} declarada en \S\ref{sec:conops} (priorización del caso núcleo).
...
\begin{itemize}
  ...
  \item recopila y procesa datos de percepción y de navegación,
...
\begin{description}
  ...
  El rover utiliza sus sensores (cámara OV5647, LiDAR de largo alcance TF02, ToF VL53L0X, ultrasónico HC-SR04 e IMU MPU-6050) para caracterizar el entorno inmediato, identificando pendientes, rocas, obstáculos y zonas transitables.

  \item[\textbf{Recopilar datos del entorno}]
  Incluye la adquisición de datos de percepción (imágenes RGB de la cámara OV5647 y medidas de distancia de los sensores ToF/ultrasónicos/LiDAR) y datos de navegación (posición y velocidad estimadas por encoders, inclinación de la IMU MPU-6050).
  ...
  Supervisión continua de variables internas (batería, temperatura, estado de actuadores, integridad de sensores, etc.) para detectar condiciones de riesgo que puedan requerir la activación de un modo seguro o la transición a otro caso de uso (por ejemplo, UC-04 \emph{Homing} o, ante fallo crítico no recuperable, UC-05 \emph{Safe Deactivation / Passivation}; estas transiciones se modelan en el state machine de \S\ref{subsec:uc01_states}).
  \end{description}
...
\begin{enumerate}
  ...
  \item \textbf{Recopilar datos del entorno}: durante la navegación, el rover adquiere datos
        de percepción (imágenes, distancias) y datos de navegación (posición,
  ...
  \item En caso de que se detecten \textbf{condiciones extremadamente adversas} (por ejemplo,
        condiciones ambientales adversas, terrenos demasiado peligrosos) o una \textbf{falla crítica} en el
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        composite/.style={rectangle, rounded corners=4pt, draw=black, very thick,
                          double, double distance=2pt,
                          minimum width=2.8cm, minimum height=1.0cm,
                          align=center, fill=blue!12},
        safety/.style={rectangle, rounded corners=3pt, draw=black, thick,
                       minimum width=2.0cm, minimum height=0.8cm,
                       align=center, fill=orange!12, font=\footnotesize},
        terminal/.style={rectangle, rounded corners=4pt, draw=red!70!black, very thick,
                         minimum width=2.4cm, minimum height=0.9cm,
                         align=center, fill=red!10},
        init/.style={circle, draw=black, fill=black, minimum size=4pt, inner sep=0pt},
        arrow/.style={-{Stealth[length=2mm]}, thick},
        ev/.style={font=\scriptsize\ttfamily, fill=white, inner sep=1pt}
    ]

    % ============== PANEL IZQUIERDO: RoverState (7 estados) ==============
    \node[font=\bfseries] (titR) at (0,2.4) {Dimensión 1 --- \texttt{RoverState} (7 estados)};

    \node[init] (init) at (-5.5,0.6) {};
    \node[state] (standby) [right=0.8cm of init] {Standby};
    \node[composite] (explore) [right=2.2cm of standby] {Explore\\[-1pt]\scriptsize\textit{(compuesto)}};
    \node[state] (avoid)   [above=1.3cm of explore] {Avoid};
    \node[state] (climb)   [right=1.6cm of explore] {Climb};
    \node[state] (retreat) [below=1.3cm of explore] {Retreat};
    \node[terminal] (safe)  at ($(climb)+(2.6,1.4)$) {Safe};
    \node[terminal] (fault) at ($(climb)+(2.6,-1.4)$) {Fault};

    % Transiciones RoverState
    \draw[arrow] (init) -- (standby);
    \draw[arrow] (standby.east) -- node[ev, above]{EXP} (explore.west);
    \draw[arrow] (explore.north west) to[bend left=15] node[ev, above]{STB} (standby.north east);

    \draw[arrow] (explore.north) -- node[ev, right]{dist$<$200\,mm} (avoid.south);
    \draw[arrow] (avoid.west) to[bend left=20] node[ev, above]{clear} (explore.north west);

    \draw[arrow] (explore.east) -- node[ev, above]{CLB} (climb.west);
    \draw[arrow] (climb.south) to[bend left=25] node[ev, below]{CLB end} (explore.south east);

    \draw[arrow] (explore.south) -- node[ev, right]{dist$<$300\,mm / stall} (retreat.north);
    \draw[arrow] (retreat.west) to[bend right=20] node[ev, below]{wp\_reached} (standby.south);

    \draw[arrow, dashed, red!70!black] (explore.north east) to[bend left=15] node[ev, above, sloped]{BATT/TEMP/FAULT} (safe.west);
    \draw[arrow, dashed, red!70!black] (explore.south east) to[bend right=15] node[ev, below, sloped]{stall persist / OC} (fault.west);
    \draw[arrow] (safe.north) to[out=180, in=90, looseness=1.4] node[ev, above]{RST} (standby.north);
    \draw[arrow] (fault.south) to[out=180, in=270, looseness=1.4] node[ev, below]{RST} (standby.south);

    % ============== PANEL DERECHO: SafetyState (4 niveles) ==============
    \node[font=\bfseries] (titS) at (10.5,2.4) {Dimensión 2 --- \texttt{SafetyState} (paralelo)};

    \node[safety, fill=green!15]  (normal) at (10.5, 0.9) {Normal};
    \node[safety, fill=yellow!25] (warn)   [below=0.5cm of normal] {Warn};
    \node[safety, fill=orange!30] (limit)  [below=0.5cm of warn]   {Limit};
    \node[safety, fill=red!25]    (faultS) [below=0.5cm of limit]  {FaultStall};

    \draw[arrow] (normal.south) -- node[ev, right]{HW alerta} (warn.north);
    \draw[arrow] (warn.south)   -- node[ev, right]{cerca umbral} (limit.north);
    \draw[arrow] (limit.south)  -- node[ev, right]{persistente} (faultS.north);
    \draw[arrow, dashed, green!50!black] (faultS.east) to[bend left=60] node[ev, right]{recover/reset} (normal.east);

    % Nota explicativa
    \node[font=\scriptsize\itshape, align=left, text width=4.2cm] at (10.5,-3.4)
      {Modula límites operacionales\\(velocidad máx, rampas) sin\\cambiar \texttt{RoverState}.\\Independiente del Panel 1.};

    % Marco visual separador
    \draw[gray!50, dashed] (8.6,-3.0) -- (8.6,2.7);

    % Leyenda
    \node[font=\scriptsize, align=left, text width=8cm] at (1.5,-3.6)
      {\textbf{Leyenda:} \begin{tabular}{@{}l@{ }l@{\quad}l@{ }l@{}}
        \tikz\node[state, minimum width=0.7cm, minimum height=0.3cm]{}; & estado simple &
        \tikz\node[composite, minimum width=0.7cm, minimum height=0.3cm]{}; & estado compuesto \\
        \tikz\node[terminal, minimum width=0.7cm, minimum height=0.3cm]{}; & estado de seguridad &
        \tikz[baseline]{\draw[arrow, dashed, red!70!black] (0,0.5ex) -- (0.8,0.5ex);} & disparo crítico \\
      \end{tabular}};

    \end{tikzpicture}
    \caption{Máquina de estados del Rover Olympus: dimensión \texttt{RoverState} (7 estados, panel izquierdo) y dimensión paralela \texttt{SafetyState} (4 niveles, panel derecho). Refleja la implementación real del módulo \texttt{state\_machine} en LLC v2.20.}
    \label{fig:maquina_estados}
\end{figure}
\end{landscape}

\textbf{Estados top-level (modos de operación):} El \emph{Master State Machine} del LLC define los siguientes \texttt{RoverState} (notación de implementación entre paréntesis):
\begin{itemize}
  \item \textbf{Standby} (\texttt{Standby}): modo de espera operacional; el rover permanece disponible para recibir programación o comandos. Ejecuta \texttt{do/ esperar\_cmd\_o\_schedule()}. \cite{UMLStateMachinesConcepts1}
  \item \textbf{Exploración activa} (\texttt{Explore}; estado compuesto): agrupa el ciclo interno de exploración mediante subestados (Evaluar, Planificar, etc.), reduciendo complejidad mediante jerarquía. \cite{CompositeStates1,UMLStateMachinesConcepts2}
  \item \textbf{EvitarObstaculo} (\texttt{Avoid}): subestado de evitación reactiva activado cuando un sensor de proximidad detecta un obstáculo dentro del umbral de emergencia (\texttt{HC\_EMERGENCY\_MM = 200\,mm} o \texttt{TOF\_EMERGENCY\_MM = 150\,mm} en el LLC); ejecuta hard-stop y maniobra de desviación antes de retomar el ciclo. \cite{UMLStateMachinesConcepts2}
  \item \textbf{Escalada} (\texttt{Climb}): modo de superación de obstáculos con umbrales relajados de proximidad (\texttt{CLB\_HC\_EMERGENCY\_MM = 60\,mm}, \texttt{CLB\_TOF\_EMERGENCY\_MM = 50\,mm}, \texttt{CLB\_STALL\_THRESHOLD = 150} ciclos) para superar rampas o rocas cercanas; activado por comando \texttt{CLB} del operador. \cite{UMLStateMachinesConcepts1}
  \item \textbf{HomingRetreat} (\texttt{Retreat}): modo de retirada corta al último waypoint seguro; activado por detección de slip persistente (RF-004-R1: \texttt{SLIP\_STALL\_FRAMES = 2} en el HLC) o por comando \texttt{cmd\_homing\_retreat} del operador. Incluye \texttt{entry/ seleccionar\_lastSafeWaypoint()} y \texttt{do/ navegar\_a\_lastSafeWaypoint(); evitar\_peligros()}. \cite{UMLStateMachinesConcepts2,SafeModeContext1}
  \item \textbf{SafeMode} (\texttt{Safe}): modo de preservación (safing); prioriza seguridad del sistema, minimiza actividad no esencial y favorece diagnósticos y comunicación mínima. Activado por anomalía de batería (LLC \texttt{batt\_mv < 12.8\,V}), temperatura ambiente $>$ \texttt{AMBIENT\_SAFE\_C}, EMC u otro fallo recuperable. \cite{SafeModeContext1,UMLStateMachinesConcepts2}
  \item \textbf{Fault} (\texttt{Fault}): estado de falla crítica no recuperable; el rover detiene actuadores y emite telemetría mínima hasta orden de \texttt{RST} y diagnóstico off-line. Equivale al pseudoestado \emph{Final} en UML cuando no se prevé recuperación dentro de la misma campaña. \cite{UMLStateMachinesConcepts2}
\end{itemize}

\textbf{Dimensión paralela --- SafetyState:} Independientemente del \texttt{RoverState}, el LLC mantiene un \texttt{SafetyState} de cuatro niveles --- \texttt{Normal}, \texttt{Warn}, \texttt{Limit}, \texttt{FaultStall} --- que modula los límites operacionales (velocidad máxima permitida, agresividad de las rampas) sin cambiar de \texttt{RoverState}. Esta separación permite degradar el desempeño de forma gradual (p.\,ej., reducir velocidad al detectar sobrecorriente en un motor) antes de invocar SafeMode o Fault.
...
Los eventos conceptuales aquí descritos se realizan a nivel de implementación sobre el protocolo ASCII del ICD rover--GCS (comandos \texttt{EXP}, \texttt{CLB}, \texttt{STB}, \texttt{RST}, entre otros), definido en \S\ref{sec:system_interfaces}.
...
\textbf{Implementación actual del gating de retreat (HLC v0).} En el stack HLC desplegado (\texttt{olympus\_hlc.monitors.WaypointTracker} y \texttt{olympus\_hlc.engine}), las guardas de severidad multi-nivel anteriores aún no están implementadas. La decisión de retreat táctico se reduce a la conjunción \texttt{tlm.dist\_mm > 0 AND tlm.dist\_mm < RETREAT\_DIST\_MM (300\,mm)}, evaluada solo si \texttt{msm\_state != CLIMB} (se suprime durante escalada porque CLB usa \texttt{CLB\_TOF\_EMERGENCY\_MM = 50\,mm}) y siempre que \texttt{SafeMode} no esté activo (SafeMode bloquea todo comando excepto \texttt{STB}, \texttt{PING}, \texttt{RST}, \texttt{BNK:0}). La prioridad ejecutada en \texttt{engine.update()} es \texttt{SafeMode > retreat táctico > slip}. La cuantificación multi-nivel de severidad queda como gap (TBD-OP-01) y se contempla integrar mediante ELANav cuando esté disponible.
...
\begin{enumerate}
    \item \textbf{Uplink (Planificación / Comandos):} El operador envía objetivos o comandos de alto nivel (p.\,ej., meta/área a explorar) y parámetros de ejecución.
    \item \textbf{Ejecución autónoma:} El rover ejecuta navegación autónoma local (percepción, clasificación de transitabilidad, evitación de obstáculos, detección de slip y retracción) y gestiona contingencias (fault stop, retorno a comunicación). En la configuración actual de validación el stack de navegación es el del HLC de Olympus; en la integración prevista con el proyecto matriz será ejecutado por la biblioteca ELANav.
    \item \textbf{Downlink (Telemetría / Datos):} El rover transmite telemetría y datos de desempeño (logs, estados, métricas relevantes) para análisis posterior.
    \item \textbf{Hibernación / Idle} \emph{[Conceptual --- modo de bajo consumo no implementado en la configuración actual; el rover retorna a Standby entre ventanas]:} reposo operativo previsto entre ventanas de actividad o condiciones del sitio.
\end{enumerate}
...
El contexto operacional para UC-01 consolida las entidades externas y las interfaces principales durante operación en sitio análogo. La frontera del sistema y entidades externas se definen en \S\ref{subsec:system_boundary}, y los actores/sistemas externos se detallan en \S\ref{subsec:actors_systems}. La Figura~\ref{fig:operational_context} resume dichas relaciones para facilitar la lectura del UC-01 y su interacción TM/CMD.
...
\begin{enumerate}
    \item \textbf{Geometría y mecánica del terreno:} Pendientes de hasta 20° y transiciones de suelo con obstáculos; estas condiciones pueden inducir slip y atascos (ver caracterización del entorno en \S\ref{subsec:operational_environment}).
    \item \textbf{Iluminación y visibilidad:} Presencia de zonas de sombra o baja iluminación que afecta percepción visual y clasificación de transitabilidad (ver §\ref{subsec:operational_environment}).
    \item \textbf{Comunicaciones con latencia simulada (CON‑002):} Inyección de latencia 0–5 s en el enlace para emular retardo operacional (ver §\ref{subsec:operational_environment} y CON-002 en requisitos).
    \item \textbf{Condiciones de campaña ``lab-only'' (CON-001):} Marco operativo controlado donde la restricción energética se simplifica respecto a una misión espacial real (ver CON-001 en requisitos).
\end{enumerate}
