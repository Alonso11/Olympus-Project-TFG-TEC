...
Dado que los documentos main/\allowbreak main1/\allowbreak main3 son iteraciones previas del mismo borrador, el objetivo de esta sección es establecer una disciplina de ``baseline único'': (i) Una definición maestra por requisito en la Sección~\ref{sec:requirements}. (ii) Referencias cruzadas por ID en ConOps/\allowbreak Interfaces/\allowbreak V\&V/\allowbreak Arquitectura. (iii) Un proceso de control de cambios proporcional al proyecto académico que evite divergencias entre copias del documento y del modelo. \cite{nasa_cm, segoldmine_ppi}
...
\textbf{Regla operativa de baseline:} La Sección~\ref{sec:requirements} contiene la definición maestra de RF/RNF/CON; ConOps, Interfaces, Arquitectura y V\&V solo referencian por ID para evitar múltiples definiciones inconsistentes entre secciones. \cite{nasa_cm, segoldmine_ppi}
