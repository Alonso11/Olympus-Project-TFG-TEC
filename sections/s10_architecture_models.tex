...
Puentes H (L298N) con PWM + Arduino Mega (control) y/o Raspberry Pi (alto nivel).\\ \hline

F13 & PROP & Permitir la inversión del sentido de giro de los actuadores &
Puentes H (L298N) + control digital desde Arduino Mega.\\ \hline

F14 & PROP & Proteger al actuador ante la entrega de potencia anómala &
Protecciones del driver (L298N) + BMS Daly 4S 20A + (recomendado) fusibles/TVS externos.\\ \hline

F15 & PROP & Limitar y regular el consumo de potencia/corriente de los actuadores &
Drivers (L298N) + control (Arduino Mega) + (recomendado) sensado de corriente externo (INA/ACS).\\ \hline
...
Raspberry Pi 5 (principal) + Raspberry Pi 4 (backup/auxiliar).\\ \hline
...
Packs con 18650 + (recomendado) cargador 4S Li-ion / fuente externa de carga.\\ \hline

F36 & EPS & Almacenar energía en un reservorio &
Pack 4S basado en baterías 18650 (8 celdas por pack).\\ \hline

F37 & EPS & Balancear y gestionar el reservorio de energía &
BMS Daly 4S 20A (Li-ion; balanceo/protección del pack).\\ \hline

F38 & EPS & Suministrar energía regulada a los subsistemas consumidores. &
Convertidores DC-DC (buck 4S$\rightarrow$5V/6V/12V según cargas) + distribución.\\ \hline

F39 & EPS & Distribuir potencia incorporando mecanismos de aislamiento/segmentación &
Power Distribution (PDB) + fusibles por rama + interruptor principal/e-stop (recomendado).\\ \hline

F40 & EPS & Implementar funciones de protección y seguridad eléctrica ante condiciones anómalas &
BMS Daly + (recomendado) fusibles/TVS/e-stop + buenas prácticas de cableado.\\ \hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\paperheight]{figures/BDD-Page-2.drawio.png}
...
El subsistema \textbf{Propulsión (PROP)} convierte potencia eléctrica en \textbf{torque mecánico utilizable} para movilidad. En el modelo físico se compone de \textbf{MotoresRueda x6}, \textbf{ControladoresVelocidad} (drivers L298N o BTS7960) y módulos de protección.

PROP recibe energía desde EPS (canal dedicado \texttt{EPS.pPowerChannel\_PROP} $\rightarrow$ \texttt{PROP.pPowerIn\_PROP}) y recibe comandos de actuación directamente desde el **Arduino Mega** (\texttt{MCU.pPWM\_Out} $\rightarrow$ \texttt{PROP.pControlIn\_Motores}). El Arduino, bajo mando de la Raspberry Pi 5, orquesta la velocidad y sentido de giro. Los eventos de diagnóstico del driver pueden reportarse localmente a la MCU para su consolidación en la telemetría del C\&DH. Esta arquitectura asegura que la ejecución del movimiento ocurra en un nodo de tiempo real, mientras que la decisión de alto nivel reside en la OBC.
...
C\&DH integra los flujos de información del sistema: la RPi 5 actúa como el \textbf{hub de percepción}, recibiendo datos directos de cámaras y sensores de rango (LiDAR/Ultrasonido), y se conecta con COMMS vía \textbf{USB (KISS)}. El Arduino Mega recibe comandos de dirección y velocidad desde la RPi 5 para gestionar los actuadores. A nivel energético, el subsistema se alimenta desde EPS (\texttt{EPS.pPowerOut\_Regulada} $\rightarrow$ \texttt{C\&DH.pPowerIn\_CDH}) y monitorea la telemetría eléctrica a través del bus I2C (sensor INA219).

\paragraph{Subsistema de Guía, Navegación y Control (GNC)}
El subsistema \textbf{GNC} implementa la autonomía de navegación: percepción del entorno, estimación de estado y generación de comandos de movimiento. En el modelo IBD, la percepción se centraliza en la \textbf{Raspberry Pi 5}, conectando directamente la \textbf{Cámara CSI}, el \textbf{LiDAR TF-Luna} (UART/I2C) y el \textbf{Sensor Ultrasónico HC-SR04} (GPIO). Esta configuración permite que los modelos de IA procesen datos de proximidad y visión con mínima latencia. 

La IMU principal y los encoders de rueda completan el esquema sensorial para satisfacer GNC-REQ-003, RF-004 y RNF-003. GNC (ejecutado en la RPi 5) genera setpoints de movilidad que se envían vía UART al \textbf{Arduino Mega}, el cual implementa la ejecución final mediante señales **PWM** hacia los drivers de motor y servos de dirección. Se añade un canal de \emph{e-stop} cableado en EPS/PDU que garantiza la detención $\le 500$\,ms conforme a PROP-REQ-002.
...
COMMS recibe alimentación desde EPS (\texttt{EPS.pPowerChannel\_COMMS} $\rightarrow$ \texttt{COMMS.pPowerIn\_COMMS}). La integración de datos se realiza mediante un enlace **USB (protocolo KISS)** entre la Raspberry Pi 5 y el TNC para el canal UHF, mientras que el flujo Wi-Fi se gestiona internamente en la OBC. Esto asegura que la gestión de paquetes CSP y el ruteo de telemetría se centralicen en el nodo de alto nivel del C\&DH. La interacción con el ambiente se modela mediante el puerto RF (\texttt{pRF\_Out}), incluyendo el enlace de video **FPV independiente** de 5.8 GHz.

\paragraph{Subsistema de Potencia Eléctrica (EPS)}
El subsistema \textbf{EPS} gestiona la energía del rover: almacenamiento, regulación y distribución. Sus componentes internos incluyen \textbf{Banco de Baterías (18650)}, \textbf{BMS}, \textbf{Convertidores DC/DC (buck)}, \textbf{PDU} (unidad de distribución), \textbf{Protecciones Eléctricas} (fusibles/breakers/switches) y \textbf{Módulo de Supervisión de Potencia} (medición de V/I y lógica de protección). Opcionalmente, puede incluir \textbf{Fuente de Generación} (p.\ ej., paneles) como entrada energética. La PDU implementará segmentación por ramas con fusibles y límites de corriente documentados en el ICD de potencia. Cada convertidor DC-DC incluirá su curva de eficiencia y su nivel de \emph{derating}. Se define una política de \emph{load-shed} para modo SAFE donde se inhabilitan secuencialmente: COMMS vídeo (FPV), GNC visión intensiva y servicios no esenciales de C\&DH.
...
\paragraph{Selección de materiales (Manufactura aditiva).}
La estructura principal (chasis, ruedas y bahía de aviónica) se fabrica mediante impresión 3D utilizando filamento \textbf{PLA MAX}. Este material ha sido seleccionado por su \textbf{alta tenacidad y rigidez}, superando al PLA estándar en resistencia a impactos y tracción, lo que garantiza la integridad estructural durante la navegación en terrenos irregulares. Presenta una densidad de 1.24\,g/cm$^3$, una temperatura de deformación de 57\,$^{\circ}\mathrm{C}$ y una precisión dimensional de $\pm$0.02\,mm, facilitando el ensamblaje de piezas de precisión como hubs y soportes de rodamientos.
...
\hline
ASM-010 & L2 & STR & Chasis estructural (marco + refuerzos) & [MEC] soporte TRAC/PROP/C\&DH/EPS/COMMS/GNC & COTS & \textbf{TBD} \\
...
ASM-030 & L2 & PROP & Motor+reductora \#1 & [PWR] EPS$\rightarrow$PROP; [CTRL] GNC/MCU$\rightarrow$driver; [MEC] TRAC & COTS & NFP-5840-31ZY-EN \\
