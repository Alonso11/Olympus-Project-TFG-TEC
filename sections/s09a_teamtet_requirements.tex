...
% (Se usan \providecommand para evitar choques si ya existen macros similares)
\providecommand{\ReqID}[1]{\textbf{ID:} #1}
\providecommand{\ReqShall}[1]{\textbf{Enunciado:} #1}
\providecommand{\ReqMetric}[1]{\textbf{Criterio cuantitativo:} #1}
\providecommand{\ReqCond}[1]{\textbf{Condición de aplicación:} #1}
\providecommand{\ReqAccept}[1]{\textbf{Criterio de aceptación:} #1}
\providecommand{\ReqTrace}[1]{\textbf{Trazabilidad:} #1}
...
\begin{reqblock}
  ...
  \item \ReqMetric{Reducción de error $\ge$ 30\% mediante fusión/estimación (EKF) respecto a la línea base.}
  \item \ReqCond{UC-01 en terreno con transiciones/pendientes.}
  \item \ReqAccept{Análisis comparativo (baseline vs EKF) que evidencie mejora $\ge$ 30\%.}
  \item \ReqTrace{CU: UC-01; Req: RF-004; Interfaces internas SW condicionadas por desempeño.}
\end{reqblock}
...
Se mantienen como \textbf{TBD} los umbrales y políticas explícitamente no cuantificados (p.\,ej., criterios de \texttt{route\_valid}, severidades de \texttt{conditions\_adverse\_detected}, políticas de reintentos \texttt{retry\_allowed/retry\_exhausted}, umbral de \texttt{storage\_near\_full}, definición de ``enlace estable'' y método de referencia para medición de error de navegación).

Las guardas basadas en severidad y políticas de reintento se consideran TBD operativos; se definirán mediante parámetros configurables en el software (config files) documentados en el ICD de software y reflejados en los procedimientos de prueba. Su valor por defecto se definirá en la configuración de campaña.
