...
\paragraph{SyRS-013 — Compensación de slip (mejora $\ge$30\%)}
\begin{tcblock}
  \item \TCID{VT-SyRS-013}
  \item \TCMethod{A/T}
  \item \TCObj{Cuantificar mejora $\ge$30\% en error usando EKF vs baseline.}
  \item \TCPre{Definición de baseline sin EKF o configuración alternativa (TBD si no existe).}
  \item \TCSetup{Recorrido replicable con transiciones; registro de estimación (con EKF y baseline); referencia (TBD).}
  \item \textbf{Procedimiento:}
    \begin{enumerate}
      \item Ejecutar recorrido en modo baseline; registrar error/métrica.
      \item Ejecutar el mismo recorrido con EKF; registrar error/métrica.
      \item Calcular porcentaje de mejora.
    \end{enumerate}
  \item \TCData{Series temporales; cálculo de error; comparación y mejora.}
  \item \TCAccept{Mejora $\ge$30\% respecto a baseline definido.}
  \item \TCTrace{UC-01; RF-004.}
\end{tcblock}
...
\begin{tcblock}
  ...
  \item \TCMethod{D/T}
  \item \TCObj{Verificar transición Any$\rightarrow$SafeMode ante sw\_fault\_critical o power\_anomaly\_detected.}
...
\begin{tcblock}
  ...
  \item \textbf{Procedimiento:}
    \begin{enumerate}
      ...
      \item Verificar emisión de storage\_near\_full y priorización de downlink cuando comms\_window\_open esté disponible.
    \end{enumerate}
...
\begin{tcblock}
  ...
  \item \textbf{Procedimiento:}
    \begin{enumerate}
      \item Verificar la firma/versión del sistema operativo y manifiestos de build (manifest/lock/hashes).
    \end{enumerate}
